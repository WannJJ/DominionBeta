...
import { ACTIVITY_PLAY_AS_WAY, PHASE_BUY } from "../utils/constants.js";
import { Activity } from "./report_save_activity.js";
...
class GameState {
  constructor() {
    ...
    this.cards_gained_this_game = []; // Use for Kintsugi
  ...
  update_activity(activity = "", card = null, cardList = []) {
    if (card == null || card.name === undefined)
      this.last_activity = { name: activity, card: undefined };
    else if (activity === "Autoplay Treasures" && Array.isArray(card)) {
      this.last_activity = {
        name: "Autoplay Treasures",
    ...
    switch (activity) {
      ...
      case "Gain":
        ...
        this.cards_gained_this_game.push(card);
      ...
      case "Buy and Gain":
        ...
        this.activities_this_turn.push({
          name: "Buy and Gain",
          card: card.name,
        });
        break;
      case "Trash":
        if (card) {
          ...
          for (let c of cardList) {
            this.cards_trashed_this_turn.push(c);
            this.activities_this_turn.push({ name: "Trash", card: c.name });
          }
        }
        break;
      case "Draw":
  ...
  get_state() {
    let state = { username: getPlayer().username };
  ...
  async undo() {
    ...
    while (last_state.move >= getPlayer().move && this.states.length > 0) {
      //this.states.pop();
      if (this.states.length === 0) return;
    ...
    Activity.current = null; //TODO: Test
  ...
  create_report() {
    ...
    report.cards_played_this_turn = this.cards_played_this_turn.map((c) =>
      c.createMockObject()
    );
    report.cards_gained_this_turn = this.cards_gained_this_turn.map((c) =>
      c.createMockObject()
    );
    report.cards_gained_this_game = this.cards_gained_this_game.map((c) =>
      c.createMockObject()
    );
    report.cards_gained_this_buy_phase = this.cards_gained_this_buy_phase.map(
      (c) => c.createMockObject()
    );

    report.cards_buyed_this_turn = this.cards_buyed_this_turn.map((c) =>
      c.createMockObject()
    );
    report.cards_trashed_this_turn = this.cards_trashed_this_turn.map((c) =>
      c.createMockObject()
    );
    ...
    report.all_cards = getPlayer().all_cards.map((card) =>
      card.createMockObject(true)
    );

    report.basicStats = getBasicStats().createMockObject();

    report.can_play_treasure_buy_phase =
      getPlayer().can_play_treasure_buy_phase;
    report.can_not_buy_action_this_turn =
      getPlayer().can_not_buy_action_this_turn;
  ...
  async parse_report(report) {
    ...
    if (report.username === getPlayer().username) {
      ...
      this.cards_played_this_turn = report.cards_played_this_turn.map(
        (cardObject) => generateCardFromMockObject(cardObject)
      );
      this.cards_gained_this_turn = report.cards_gained_this_turn.map(
        (cardObject) => generateCardFromMockObject(cardObject)
      );
      this.cards_gained_this_game = report.cards_gained_this_game.map(
        (cardObject) => generateCardFromMockObject(cardObject)
      );
      this.cards_gained_this_buy_phase = report.cards_gained_this_buy_phase.map(
        (cardObject) => generateCardFromMockObject(cardObject)
      );
      this.cards_buyed_this_turn = report.cards_buyed_this_turn.map(
        (cardObject) => generateCardFromMockObject(cardObject)
      );
      this.cards_trashed_this_turn = report.cards_trashed_this_turn.map(
        (cardObject) => generateCardFromMockObject(cardObject)
      );
      ...
      await landscapeEffectManager.parseDataFromMockObjectOwn(
        report.landscapeEffectManager
      );
      ...
      await getNativeVillageMat().parseDataFromMockObject(
        report.nativeVillageMat
      );
      ...
      getPlayer().is_attacked_by_enchantress =
        report.is_attacked_by_enchantress;
      ...
      await landscapeEffectManager.parseDataFromMockObjectGeneral(
        report.landscapeEffectManager
      );
