...
const REASON_SHUFFLE = 'activate_when_shuffle', // Order of masons, order of astroloders, fated, stash, star chart, Emissary, Avoid
    ...
    REASON_WHEN_DISCARD_FROM_PLAY = "activate_when_discard_from_play",
    ...
    REASON_DISCARD_FROM_PLAY // Trickster, Capital,
    REASON_DISCARD_OTHER_THAN_CLEAN_UP
...
const reactionEffectManager = {
    get_possible_cards: function(reason, card, activity){
        ...
        for(let c of getHand().getCardAll()){
            if(c.activate_when_in_hand && c[reason] && c.should_activate(reason, card, activity)){
                cardList.push({card: c, position: POSITION_HAND});
            }
        }
        for(let c of getPlayField().getCardAll()){
            if(c.activate_when_in_play && c[reason] && c.should_activate(reason, card, activity)){
        ...
        for(let landscapeEffectComponent of landscapeEffectManager.getEffectComponentListAll()){
            ...
            if((effectCard.activate_permanently || effectCard.activate_currently) && effectCard[reason] && effectCard.should_activate(reason, card, activity)){
                cardList.push({card: effectCard, position: POSITION_LANDSCAPE_EFFECT});
            }
        }
        
        for(let boonCard of boonHolder.getBoonList()){
            if(boonCard.activate_currently && boonCard[reason] && boonCard.should_activate(reason, card, activity)){
                cardList.push({card: boonCard, position: POSITION_BOON_HOLDER});
            }
        } 
        for(let state of stateHolder.getStateList()){
            if(state.activate_currently && state[reason] && state.should_activate(reason, card, activity)){
    ...
    solve_cards_when_play: async function(card){
        if(!card) return;
    ...
    */
    solve_cards_when_gain: async function(card, activity, cardLocation){
        if(!card) return;
        await this.solve_remaining_effects(REASON_WHEN_GAIN, card, activity, cardLocation);
    },
    ...
    solve_cards_when_shuffle: async function(cardList){
        await this.solve_remaining_effects(REASON_SHUFFLE, cardList);
    },
    ...
    solve_cards_when_another_gains: async function(card, activity){ 
        if(!card) return;
        await this.solve_remaining_effects(REASON_WHEN_ANOTHER_GAIN, card, activity);
    },
    solve_cards_first_when_another_plays: async function(card, activity){
        if(!card) return;
        await this.solve_remaining_effects(REASON_FIRST_WHEN_ANOTHER_PLAYS, card, activity);
    }, 
    ...
    solve_remaining_effects: async function(reason, card, activity, location){ 
        ...
        while(card_list.length > 0){
            ...
            if(card_list.length === 1){
                ...
                await this.solve_single_remaining_effect(c_object, reason, card, activity, cardLocationTrack);
            } else{
                await this.solve_multiple_remaining_effect(card_list, reason, card, activity, cardLocationTrack);
            }
        }
    },
    solve_single_remaining_effect: async function(c_object, reason, card, activity, cardLocationTrack){
        let duration_card = c_object.card,
            position = c_object.position;

        if(duration_card.should_activate(reason, card, activity, cardLocationTrack)){ //cardLocation is unneccessary
            if(duration_card.type.includes(Card.Type.REACTION) && position === POSITION_HAND){
                //ask wether player wants to activate
            } else{
                await this.remove_card_from_position(duration_card, position);
            }
            await activate_card(duration_card, reason, card, activity, cardLocationTrack);
        }
    },
    solve_multiple_remaining_effect: function(card_list, reason, card, activity, cardLocationTrack){           
        ...
        return new Promise((resolve) =>{
            for(let i=0; i<card_list.length; i++){
                ...
                let duration_card = c_object.card,
                    position = c_object.position;
                if(!duration_card.should_activate(reason, card, activity, cardLocationTrack)) continue;//cardLocation is unneccessary
                getButtonPanel().add_button(`${ACTIVITY_ACTIVATE} ${duration_card.name}`, 
                    async function(){   
                        getButtonPanel().clear_buttons();
                        if(duration_card.should_activate(reason, card, activity, cardLocationTrack)){//cardLocation is unneccessary
...
const effectBuffer = {
    ...
    createMockObject(){
        let cardList = [];
        for(let card of this.cardList){
            if(!(card instanceof RootCard)) throw new Error('INVALID Card in effectBuffer');
            cardList.push(card.createMockObject());
        }
        return {type: "effectBuffer",
            name: "effectBuffer",
            cardList: cardList};
    },
    async parseDataFromMockObject(mockObj){
        if(!mockObj || mockObj.name !== "effectBuffer" || !Array.isArray(mockObj.cardList)){
...
class LocationTrack{
    ...
    getLocation(){ // getExpectedLocation
...
async function activate_card(duration_card, reason, card, activity, cardLocationTrack){
...
export {reactionEffectManager,effectBuffer,
    REASON_SHUFFLE, REASON_START_TURN, REASON_START_BUY, REASON_END_BUY, REASON_START_CLEANUP, REASON_END_TURN, REASON_END_GAME, 
    REASON_WHEN_PLAY, REASON_WHEN_GAIN, 
    REASON_WHEN_DISCARD, REASON_WHEN_DISCARD_FROM_PLAY,
     REASON_WHEN_TRASH,
    //REASON_WHEN_BEING_ATTACKED, 
    REASON_WHEN_ANOTHER_GAIN, REASON_FIRST_WHEN_ANOTHER_PLAYS};
