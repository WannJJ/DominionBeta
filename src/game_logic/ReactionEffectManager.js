...
class ReactionEffectManager{
    ...
    get_possible_cards(reason, card){
        ...
        for(let landscapeEffectComponent of getTableSide().getLandscapeEffectManager().effect_list){
            let effect_card = landscapeEffectComponent.getCard();
        ...
        for(let boonCard of boonHolder.getBoonList()){
            if(boonCard.activate_currently && boonCard[reason] && boonCard.should_activate(reason, card)){
                card_list.push({card: boonCard, position: POSITION_BOON_HOLDER});
            }
        } 
        for(let state of stateHolder.getStateList()){
            if(state.activate_currently && state[reason] && state.should_activate(reason, card)){
                card_list.push({card: state, position: POSITION_STATE_HOLDER});
            }
        } 
        return card_list;
    }
        ...
        while(card_list.length > 0){
            ...
            if(card_list.length == 1){
                ...
                if(duration_card.should_activate(reason, card)){
                    ...
                    await activate_card(duration_card, reason, card);
                }
            } else{
...
async function activate_card(duration_card, reason, card){
    report_save_activity('Activate', duration_card); 
    await duration_card.activate(reason, card);
    await report_save_activity2('Activate', duration_card);
}
