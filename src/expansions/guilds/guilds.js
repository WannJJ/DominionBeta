  ...
  play_step1(card_cost) {
    ...
    return new Promise((resolve) => {
      let is_marked = markSupplyPile(
        function (pile) {
          return (
            card_cost.isGreaterThan(pile.getCost()) &&
            pile.getQuantity() > 0 &&
            !chosen_list.includes(pile.getName()) &&
            chosen_list.length < 2
          );
        },
        async function (pile) {
  ...
  is_buyed_step1(card_cost) {
    ...
    return new Promise((resolve) => {
      let is_marked = markSupplyPile(
        function (pile) {
          return (
            card_cost.isEqual(pile.getCost()) &&
            pile.getQuantity() > 0 &&
            chosen < 2
          );
        },
...
class Advisor extends Card {
  ...
  async play() {
    ...
    if (
      getDeck().getLength() <= 0 ||
      opponentManager.getOpponentList().length <= 0
    )
      return;

    let cardList = [];
    while (cardList.length < 3 && getDeck().getLength() > 0) {
      let card = await getDeck().pop();
      cardList.push(card);
    }
    ...
    let leftPlayerName = leftPlayer.username;
    let chosenName = await message_other(
      this,
      `${cardList.map((card) => card.name).join(",")}`,
      leftPlayerName
    );
    supportHand.clear();

    for (let card of cardList) {
      if (card.name === chosenName) {
  ...
  receive_message(mess) {
    let cardList = mess
      .split(",")
      .map((cardName) => getClassFromName(cardName))
...
class Herald extends Card {
  ...
  is_buyed_step1(value) {
    ...
    return new Promise(async (resolve) => {
      ...
      supportHand.getCardAll().forEach((card) => (card.herald = false));
      getButtonPanel().clear_buttons();
      getButtonPanel().add_button("OK", async function () {
...
class Plaza extends Card {
  ...
  async play() {
    ...
    return new Promise((resolve) => {
      ...
      getButtonPanel().clear_buttons();
      setInstruction("Plaza: You may discard a Treasure for +1 Coffer.");
...
class Taxman extends Card {
  ...
  play() {
    ...
    return new Promise((resolve) => {
      ...
      getButtonPanel().clear_buttons();
      setInstruction("You may trash a Treasure from your hand.");
  ...
  play_step1(card_cost) {
    return new Promise((resolve) => {
      markSupplyPile(
        ...
        async function (pile) {
          removeMarkSupplyPile();
          let card = await gain_card(pile, getDeck());
          resolve();
        }
      );
    });
  ...
  is_attacked(message) {
    ...
    return new Promise((resolve) => {
      let is_marked = getHand().mark_cards(
        function (card) {
          return card.name === message && chosen === 0;
        },
        async function (card) {
          getHand().remove_mark();
          await discard_card(card);
      ...
      markSupplyPile(
        ...
        async function (pile) {
          ...
          if (pile.getCost().coin > card_cost.coin) {
            await getBasicStats().addCoffer(
              card_cost.coin - pile.getCost().coin
            );
          }
...
class MerchantGuild extends Card {
  ...
  async activate(reason, card) {
    ...
    await getBasicStats().addCoffer(
      getPlayer().gameState.cards_gained_this_buy_phase.length
    );
  }
}
