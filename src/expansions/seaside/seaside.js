  ...
  async play() {
    ...
    if (getHand().length() >= 0) {
      ...
      await new Promise((resolve) => {
        ...
        getHand().mark_cards(
          function (card) {
            return chosen === 0 && !this.chosen_id;
          }.bind(this),
          async function (card) {
            getHand().remove_mark();
...
class Lighthouse extends Card {
  constructor() {
    ...
    this.description =
      "At the start of your next turn: +$1. Until then, when another player plays an Attack card, it doesnt affect you.";
  }
  async play() {
    this.not_discard_in_cleanup = true;
...
class Monkey extends Card {
  ...
  should_activate(reason, card, activity) {
    return (
      ...
      (REASON_WHEN_ANOTHER_GAIN &&
        card &&
        opponentManager.getRightPlayer().username === activity.username)
    );
  }
...
class Smugglers extends Card {
  ...
  play() {
    ...
    setInstruction(
      "Gain a card that the player to your right gained on their last turn"
    );
    ...
    if (
      !findSupplyPile(
        (pile) =>
          pile.getQuantity() > 0 &&
          nameList.includes(pile.getNextCard().name) &&
...
class Blockade extends Card {
  ...
  should_activate(reason, card) {
    ...
    if (
      ...
      !effectBuffer.getCardById(this.id) ||
      !this.chosen_name
    ) {
      this.removeSelf();
      return false;
...
class Island extends Card {
  constructor() {
    super(
      "Island",
      new Cost(4),
      Card.Type.ACTION + " " + Card.Type.VICTORY,
      "Seaside/"
    );
  }
  getInitAmount() {
...
class Sailor extends Card {
  constructor() {
    ...
    this.activate_when_gain = false;
    this.description =
      "Once this turn, when you gain a Duration card, you may play it. At the start of your next turn, +$2 and you may trash a card from your hand.";
  }
  async play() {
    this.not_discard_in_cleanup = true;
    this.activate_when_in_play = true;
      ...
      getHand().mark_cards(
        ...
        async function (card) {
          if (chosen === 0) {
            ...
            await getBasicStats().addCoin(getCost(card).coin);
            await trash_card(card);
...
class Pirate extends Card {
  constructor() {
    ...
    this.description =
      "At the start of your next turn, gain a Treasure costing up to $6 to your hand. When any player gains a Treasure, you may play this from your hand.";
    this.activate_when_start_turn = false;
    this.activate_when_gain = true;
    this.activate_when_another_gains = true;
    this.activate_when_in_play = true;
...
class Tactician extends Card {
  ...
  async play() {
    if (getHand().length() > 0) {
      await discardCardList(getHand().getCardAll());
...
class Treasury extends Card {
  ...
  is_end_buy_phase() {
    if (
      !getGameState().cards_gained_this_turn.find((card) =>
