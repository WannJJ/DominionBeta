...
class Delay extends Event {
  constructor() {
    super("Delay", new Cost(0), "Menagerie/Event/");
    this.turn = -1;
    this.chosen_id = null;
  ...
  should_activate(reason, card) {
    return (
      ...
      this.chosen_id &&
      this.activate_currently &&
      getPlayer().turn === this.turn + 1 &&
...
class Desperation extends Event {
  constructor() {
    super("Desperation", new Cost(0), "Menagerie/Event/");
  }
  is_buyed() {
    if (
      getPlayer().gameState.cards_buyed_this_turn.filter(
        (c) => c.name === this.name
      ).length > 1
    )
      return;
    let curse_pile = findSupplyPile(function (pile) {
      return pile.getQuantity() > 0 && pile.getName() === "Curse";
    });
...
class Gamble extends Event {
  constructor() {
    super("Gamble", new Cost(2), "Menagerie/Event/");
  }
  async is_buyed() {
    await getBasicStats().addBuy(1);
    if (getDeck().length() <= 0) {
...
class Pursue extends Event {
  constructor() {
    super("Pursue", new Cost(2), "Menagerie/Event/");
    this.description =
...
class Ride extends Event {
  constructor() {
    super("Ride", new Cost(2), "Menagerie/Event/");
  }
  async is_buyed() {
...
class Toil extends Event {
  constructor() {
    super("Toil", new Cost(2), "Menagerie/Event/");
  }
  async is_buyed() {
    await getBasicStats().addBuy(1);
    if (getHand().length() <= 0) return;
...
class Enhance extends Event {
  constructor() {
    super("Enhance", new Cost(3), "Menagerie/Event/");
  }
  is_buyed() {
    if (getHand().length() <= 0) return;
    return new Promise((resolve) => {
      ...
      getButtonPanel().clear_buttons();
      setInstruction(
        "Enhance: You may trash a non-Victory card from your hand."
      );
      ...
      let is_marked = getHand().mark_cards(
        function (card) {
          return (
            this.chosen === 0 && !getType(card).includes(Card.Type.VICTORY)
          );
        }.bind(this),
        async function (card) {
          clearFunc();
...
class March extends Event {
  constructor() {
    super("March", new Cost(3), "Menagerie/Event/");
  }
  is_buyed() {
    ...
    return new Promise(async (resolve) => {
      ...
      getButtonPanel().add_button("Cancel", async function () {
        await supportHand.setCardAll(
          supportHand.state.cards.filter((card) => !card.march)
        );
...
class Transport extends Event {
  constructor() {
    super("Transport", new Cost(3), "Menagerie/Event/");
    this.description =
  ...
  is_buyed() {
    return new Promise((resolve) => {
      ...
      if (
        ...
        getExile()
          .getCardAll()
          .find((c) => getType(c).includes(Card.Type.ACTION))
      ) {
        getButtonPanel().add_button(
          "Put from Exile",
          async function () {
  ...
  play_step1() {
    return new Promise((resolve) => {
      markSupplyPile(
        function (pile) {
          return pile.getQuantity() > 0 && pile.getType().includes("Action");
        },
        async function (pile) {
          removeMarkSupplyPile();
...
class Banish extends Event {
  constructor() {
    super("Banish", new Cost(4), "Menagerie/Event/");
  }
  is_buyed() {
    if (getHand().length() <= 0) return;
    return new Promise((resolve) => {
      ...
      getHand().mark_cards(
        function (card) {
          return (this.chosen === 0 && name === "") || name === card.name;
        }.bind(this),
        function (card) {
          this.chosen += 1;
          if (name === "") {
...
class Bargain extends Event {
  constructor() {
    super("Bargain", new Cost(4), "Menagerie/Event/");
  }
  is_buyed() {
    return new Promise((resolve) => {
...
class Invest extends Event {
  static MESSAGE_INVEST = "Invest card";
  static MESSAGE_OK = "Ok";
  constructor() {
    super("Invest", new Cost(4), "Menagerie/Event/");
  ...
  async receive_message(mess) {
    console.log("receive mesage", mess);
    if (!mess.startsWith(Invest.MESSAGE_INVEST)) return Invest.MESSAGE_OK;
    let cardName = mess.substring(Invest.MESSAGE_INVEST.length);
    let investComponent = landscapeEffectManager.find_card(
      (component) => component.getName() === this.name
    );
    if (!investComponent) return;
    let investCard = investComponent.getCard();
    if (
      getExile().has_card(
        (card) =>
          investCard.chosen_id_list.includes(card.id) && card.name === cardName
      )
    )
      await drawNCards(2);
    return Invest.MESSAGE_OK;
  }
    ...
    return new Promise((resolve) => {
      markSupplyPile(
        ...
        async function (pile) {
          ...
          this.activate_currently = true;
          this.chosen_id_list.push(new_card.id);
          await message_other(this, `${Invest.MESSAGE_INVEST}${new_card.name}`);
  ...
  should_activate(reason, card) {
    ...
    if (
      ...
      !this.activate_currently ||
      !getExile().has_card((c) => this.chosen_id_list.includes(c.id))
    ) {
      console.log("not activate currently", this.chosen_id_list);
      this.activate_currently = false;
      return false;
    }
    ...
    console.log(
      "exile has card",
      !getExile().has_card(
        (c) => c.name === card.name && this.chosen_id_list.includes(c.id)
      )
    );
    return (
...
class Seize_the_Day extends Event {
  constructor() {
    super("Seize_the_Day", new Cost(4), "Menagerie/Event/");
...
class Commerce extends Event {
  constructor() {
    super("Commerce", new Cost(5), "Menagerie/Event/");
  }
  async is_buyed() {
    let cards_gained_this_turn =
      getPlayer().gameState.cards_gained_this_turn.map((c) => c.name);
...
class Demand extends Event {
  constructor() {
    super("Demand", new Cost(5), "Menagerie/Event/");
  }
  async is_buyed() {
...
class Stampede extends Event {
  constructor() {
    super("Stampede", new Cost(5), "Menagerie/Event/");
  }
  async is_buyed() {
...
class Reap extends Event {
  constructor() {
    super("Reap", new Cost(7), "Menagerie/Event/");
    this.turn = -1;
    this.activate_when_start_turn = true;
    this.activate_currently = false;
    this.card = null;
  ...
  async is_buyed() {
    ...
    if (gold && getDiscard().getCardById(gold.id)) {
      let removed = await getDiscard().removeCardById(gold.id);
      if (removed) {
        this.turn = getPlayer().turn;
        this.activate_currently = true;
...
class Enclave extends Event {
  constructor() {
    super("Enclave", new Cost(8), "Menagerie/Event/");
  }
  async is_buyed() {
    await gain_card_name("Gold");
    let duchy_pile = findSupplyPile(function (pile) {
      return pile.getName() === "Duchy" && pile.getQuantity() > 0;
    });
...
class Alliance extends Event {
  constructor() {
    super("Alliance", new Cost(10), "Menagerie/Event/");
  }
  async is_buyed() {
...
class Populate extends Event {
  constructor() {
    super("Populate", new Cost(10), "Menagerie/Event/");
  }
  async is_buyed() {
    let action_pile_list = findSupplyPileAll(
      (pile) =>
        pile.getQuantity() > 0 && pile.getType().includes(Card.Type.ACTION)
    );
