...
class Amphora extends Card {
  constructor() {
    super(
      "Amphora",
      new Cost(7),
      Card.Type.TREASURE + " " + Card.Type.DURATION + " " + Card.Type.LOOT,
      "Plunder/Loot/"
    );
...
class Doubloons extends Card {
  constructor() {
    super(
      "Doubloons",
      new Cost(7),
      Card.Type.TREASURE + " " + Card.Type.LOOT,
      "Plunder/Loot/"
    );
  }
  async play() {
...
class Insignia extends Card {
  ...
  should_activate(reason, card, activity, cardLocationTrack) {
    return (
      reason === REASON_WHEN_GAIN && card && getPlayer().turn === this.turn
    );
    ...
    return new Promise(async (resolve) => {
      ...
      getButtonPanel().add_button("+1 Buy +3$", async function () {
        ...
        supportHand.setCardAll(
          supportHand.state.cards.filter((card) => !card.orb)
        );
...
class Shield extends Card {
  constructor() {
    super(
      "Shield",
      new Cost(7),
      Card.Type.TREASURE + " " + Card.Type.REACTION + " " + Card.Type.LOOT,
      "Plunder/Loot/"
    );
  ...
  activate(reason, card) {
    return new Promise((resolve) => {
      ...
      getButtonPanel().clear_buttons();
      setInstruction(
        "Shield: You may reveal Shield to be unaffected by Attack."
      );
...
class SpellScroll extends Card {
  constructor() {
    super(
      "SpellScroll",
      new Cost(7),
      Card.Type.ACTION + " " + Card.Type.TREASURE + " " + Card.Type.LOOT,
      "Plunder/Loot/"
    );
  }
  async play() {
    ...
    await new Promise((resolve) => {
      markSupplyPile(
        ...
        async function (pile) {
          ...
          if (
            new_card &&
            (getType(new_card).includes(Card.Type.ACTION) ||
              getType(new_card).includes(Card.Type.TREASURE)) &&
...
class Sword extends Card {
  constructor() {
    super(
      "Sword",
      new Cost(7),
      Card.Type.TREASURE + " " + Card.Type.ATTACK + " " + Card.Type.LOOT,
      "Plunder/Loot/"
    );
  }
  async play() {
