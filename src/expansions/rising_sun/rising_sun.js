...
class Alley extends Card{
    constructor(){
        super("Alley", new Cost(4), `${Card.Type.ACTION} ${Card.Type.SHADOW}`, "RisingSun/");
    }
    async play(){
        await draw1();
...
class Change extends Card{
    constructor(){
        ...
        this.description = "If you have any D, +$3. Otherwise, trash a card from your hand, and gain a card costing more $ than it. +D equal to the difference in $.";
    }
    async play(){
        ...
        return new Promise((resolve) => {
            ...
            getHand().mark_cards(
                ...
                async function(card){
                    ...
                    markSupplyPile(
                        function(pile){
                            return pile.getCost().getCoin() > card.cost.getCoin() && pile.getQuantity()>0;
                        },
                        async function(pile){
                            let coinDiff = pile.getCost().getCoin() - card.cost.getCoin();
...
class Fishmonger extends Card{
    constructor(){
        super("Fishmonger", new Cost(2), `${Card.Type.ACTION} ${Card.Type.SHADOW}`, "RisingSun/");
    }
    async play(){
...
class GoldMine extends Card{
    ...
    async play(){
        ...
        return new Promise((resolve) =>{
            ...
            getButtonPanel().add_button("Gain Gold", 
                async function(){
                    getButtonPanel().clear_buttons();
...
class Kitsune extends Card{
    constructor(){
        super("Kitsune", new Cost(5), `${Card.Type.ACTION} ${Card.Type.ATTACK} ${Card.Type.OMEN}`, "RisingSun/");
    }
    async play(){
        await getBasicStats().addSunToken(1);

        await new Promise((resolve) =>{
            getButtonPanel().clear_buttons();
            getButtonPanel().add_button("+2 Actions", 
                async function(){
            ...
            getButtonPanel().add_button("+2 Coins",
                async function(){
...
class MountainShrine extends Card{
    constructor(){
        super("MountainShrine", new Cost(0, 5), `${Card.Type.ACTION} ${Card.Type.OMEN}`, "RisingSun/");
    }
    async play(){
        await getBasicStats().addSunToken(1);
        await getBasicStats().addCoin(2);
        ...
        return new Promise((resolve) =>{
            ...
            getHand().mark_cards(
                ...
                async function(card){
                    await clearFunc();
                    await trash_card(card);
                    if(getTrash().has_card(card1 => card1.type.includes('Action'))){
                        await drawNCards(2);
                    }
                    resolve('MountainShrine finish');
                },
...
class Ninja extends Card{
    constructor(){
        super("Ninja", new Cost(4), `${Card.Type.ACTION} ${Card.Type.ATTACK} ${Card.Type.SHADOW}`, "RisingSun/");
    }
    async play(){
        await draw1();
...
class Poet extends Card{
    constructor(){
        super("Poet", new Cost(4), `${Card.Type.ACTION} ${Card.Type.OMEN}`, "RisingSun/");
    }
    async play(){
        await getBasicStats().addSunToken(1);
        await draw1();
        ...
        let cost = new Cost(3);
        if(cost.isGreaterOrEqual(card.cost)){
            ...
            getHand().mark_cards(
                ...
                async function(card){
                    ...
                    if(card.type.includes(Card.Type.TREASURE)){
                        await drawNCards(2);
                    } else if(card.type.includes(Card.Type.ACTION)){
                        await drawNCards(5);
                    }
...
class RiverShrine extends Card{
    constructor(){
        super("RiverShrine", new Cost(4), `${Card.Type.ACTION} ${Card.Type.OMEN}`, "RisingSun/");
    ...
    async play(){
        ...
        await new Promise((resolve) => {
            ...
            getHand().mark_cards(
                function(){return getHand().getSelectedList().length<2;}, 
                function(card){
    ...
    should_activate(reason, card){
        return reason === REASON_START_CLEANUP && getGameState().cards_gained_this_buy_phase.length === 0;
    }
...
class Ronin extends Card{
    constructor(){
        super("Ronin", new Cost(5), `${Card.Type.ACTION} ${Card.Type.SHADOW}`, "RisingSun/");
    }
    async play(){
...
class RusticVillage extends Card{
    constructor(){
        super("RusticVillage", new Cost(4), `${Card.Type.ACTION} ${Card.Type.OMEN}`, "RisingSun/");
    }
    async play(){
        await getBasicStats().addSunToken(1);
        await draw1();
...
class Samurai extends Card{
    constructor(){
        super("Samurai", new Cost(6), `${Card.Type.ACTION} ${Card.Type.DURATION} ${Card.Type.ATTACK}`, "RisingSun/");
        this.not_discard_in_cleanup = true;
        this.activate_when_in_play = true;
        this.activate_when_start_turn = true;
    }
    ...
    is_attacked(){
        ...
        return new Promise((resolve) => {
            this.chosen = 0;
            let n = Math.max(getHand().length() - 3, 0);
            getHand().getCardAll().forEach(c => c.samurai=undefined);
            let clearFunc = async function(){
                getButtonPanel().clear_buttons();
                await getHand().remove_mark();
            }
...
class SnakeWitch extends Card{
    constructor(){
        super("SnakeWitch", new Cost(2), `${Card.Type.ACTION} ${Card.Type.ATTACK}`, "RisingSun/");
    }
    async play(){
        await draw1();
        await getBasicStats().addAction(1);
        
        let nameSet = new Set(getHand().getCardAll().map(card => card.name));
    ...
    play_step1(){
        return new Promise((resolve) =>{
            getButtonPanel().clear_buttons();
            getButtonPanel().add_button("Reveal Hand",
                async function(){
                    getButtonPanel().clear_buttons();
                    await reveal_card(getHand().getCardAll());
                    let snakeWitchPile = findSupplyPile(pile => pile.getName()===this.name);
...
class Tanuki extends Card{
    constructor(){
        super("Tanuki", new Cost(5), `${Card.Type.ACTION} ${Card.Type.SHADOW}`, "RisingSun/");
    }
    async play(){
        if(getHand().getLength() <= 0) return;
        return new Promise((resolve) => {
...
class TeaHouse extends Card{
    constructor(){
        super("TeaHouse", new Cost(5), `${Card.Type.ACTION} ${Card.Type.OMEN}`, "RisingSun/");
    }
    async play(){
        await getBasicStats().addSunToken(1);
        await draw1();
...
export {Alley, Aristocrat, Artist, Change, CraftsMan, Daimyo, Fishmonger, GoldMine, ImperialEnvoy,
