...
const landscapeEffectManager = {
  ...
  addComponent: function (landscapeCard) {
    ...
    if (
      this.effectComponentList.find(
        (card) => card.getName() === landscapeCard.getName()
      )
    )
      return;
  ...
  has_card: function (crit_func) {
    if (this.effectComponentList.length <= 0) {
      return false;
    }
  ...
  isValidMockObject: function (mockObj) {
    if (
      !mockObj ||
      !mockObj.effectComponentList ||
      !Array.isArray(mockObj.effectComponentList)
    ) {
  ...
  parseDataFromMockObject: async function (mockObj) {
    ...
    for (let mockCard of mockObj.effectComponentList) {
      let effectComponent = this.find_card(
        (c) => c.getName() === mockCard.name
      );
      if (effectComponent) {
